# setup-paperclip.md — ติดตั้ง opbstack บน Paperclip

> คู่มือนี้อธิบายวิธีใช้ **opbstack** (10 AI Skills สำหรับ Thai SME) ร่วมกับ **Paperclip** (Control Plane สำหรับบริษัท AI)  
> *This guide explains how to deploy opbstack's 10 AI skills on Paperclip — the control plane for AI companies.*
...
## Paperclip คืออะไร? ทำไมถึงเหมาะกับ opbstack?
...
**opbstack** = "คู่มือพนักงาน 10 เล่ม" ที่บอกว่า:
...
│   │  Code    │  │          │  │  Agent   │  │ Process  │  │
│   └──────────┘  └──────────┘  └──────────┘  └──────────┘  │
│   ← opbstack SKILL.md ทำงานในแต่ละ adapter นี้        │
└──────────────────────────────────────────────────────────────┘
```
...
## ขั้นตอนที่ 2: โหลด opbstack Skills เข้า Paperclip

### 2.1 Clone opbstack Repo

```bash
# Clone repo (หรือ copy จากที่คุณ download มา)
git clone https://github.com/apiasak/opbstack.git opbstack
cd opbstack
...
# คัดลอกทั้ง 10 skills จาก opbstack
# (ใช้ชื่อ skill ตามที่ Paperclip รองรับ — ไม่มี "/" นำหน้า)

cp -r opbstack/strategist ~/.paperclip/skills/thaistrategist
cp -r opbstack/brand ~/.paperclip/skills/thaibrand
cp -r opbstack/build ~/.paperclip/skills/thaibuild
cp -r opbstack/market ~/.paperclip/skills/thaimarket
cp -r opbstack/content ~/.paperclip/skills/thaicontent
cp -r opbstack/data ~/.paperclip/skills/thaidata
cp -r opbstack/serve ~/.paperclip/skills/thaiserve
cp -r opbstack/admin ~/.paperclip/skills/thaiadmin
cp -r opbstack/money ~/.paperclip/skills/thaimoney
cp -r opbstack/coach ~/.paperclip/skills/thaicoach

# คัดลอก context files
cp -r opbstack/core ~/.paperclip/
cp -r opbstack/business_context ~/.paperclip/
cp -r opbstack/project_context ~/.paperclip/
...
# ส่วน business/project context ของ opbstack แนะนำให้เก็บเป็นไฟล์ใน repo/workspace
...
cp -r /path/to/opbstack/strategist ~/.paperclip/skills/thaistrategist
```
